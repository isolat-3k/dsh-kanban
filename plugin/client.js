// DSH Kanban 看板插件 — Client 半（DSH Kanban board plugin, Client half）
// 运行环境：动态 Cordis Plugin 的 code.client（纯 JavaScript 函数体，无 import/TS/JSX）
// 入口：sidebar.footer.action（看板开关）+ shell.overlay（看板窗口）
...
return {
  ...
  apply(ctx) {
    ...
    const CSS = [
      ...
      '.kbn-status-row { display:flex; flex-wrap:wrap; gap:6px; }',
      '.kbn-status-btn { display:flex; align-items:center; gap:5px; border:1px solid var(--dsw-alias-border-l1); background:var(--dsw-alias-bg-layer-1); color:var(--dsw-alias-label-secondary); border-radius:12px; padding:3px 9px; font-size:11.5px; cursor:pointer; }',
      '.kbn-status-btn.on { border-color:var(--tone, var(--dsw-alias-border-l2)); color:var(--dsw-alias-label-primary); background:var(--dsw-alias-bg-layer-2); }',
    ...
    function Lane(props) {
      ...
      const head = h('div', { className: 'kbn-lane-head', onClick: () => props.onToggleCollapse(props.lane.id) },
        ...
        h('button', { className: 'kbn-icon-btn', title: '在此列新建任务', onClick: e => { e.stopPropagation(); props.onNewTask(props.lane.id) } }, '＋'),
      )
      ...
      return h('div', {
        ...
        h('div', { className: 'kbn-lane-body' },
          props.shown.length === 0 ? h('div', { className: 'kbn-lane-empty' }, props.laneTasks.length > 0 ? '无匹配任务' : '拖入任务到这里') : null,
      ...
      return h('div', { className: 'kbn-drawer' },
        ...
        h('div', { className: 'kbn-drawer-scroll' },
          ...
          h('div', { className: 'kbn-field' },
            ...
            h('div', { className: 'kbn-status-row' },
              STATUSES.filter(s => s.id !== 'running').map(s => h('button', {
                key: s.id,
                className: 'kbn-status-btn' + (s.id === task.status ? ' on' : ''),
                style: { '--tone': s.tone },
                disabled: busy || s.id === task.status,
                title: '移动到「' + s.label + '」',
                onClick: () => act(() => call('moveTask', { slug: props.slug, id: task.id, status: s.id })),
              },
    ...
    function BoardContent() {
      ...
      function doMove(id, status) {
        call('moveTask', { slug, id, status }).then(refresh).catch(e => setError(String((e && e.message) || e)))
      }
      ...
      function doBulkDelete() {
        if (selIds.length === 0) return
        call('bulkDelete', { slug, ids: selIds }).then(() => { setSel({}); refresh() }).catch(e => setError(String((e && e.message) || e)))
      }
      function doDeleteBoard() {
        call('deleteBoard', { slug }).then(() => { setConfirmBoardDel(false); setDrawerId(null); setSel({}); refresh() }).catch(e => setError(String((e && e.message) || e)))
      }
